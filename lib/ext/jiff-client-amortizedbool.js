(function (exports, node) {
  ...
  let amortFalse = 0;
  const amortTrue = null;

  let operationNum = 0;


  function convert(secret) {
    return secret.times(amortTrue);
  }

  function revert() {
    return this.cgteq(amortTrue);
  }
  ...
  function amortizedBool(instance, share) {
    ...
    share.revert = revert;
    share.amortizedOpen = amortizedOpen;
    return share;
  }
  ...
  function parseOptions(base_instance, options) {
   ...
   amortTrue = base_instance.Zp.squareRoot().floor();
   console.log('amortized false: ', amortTrue)
  }
